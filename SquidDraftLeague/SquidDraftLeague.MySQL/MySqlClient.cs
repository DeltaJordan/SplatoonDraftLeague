...
namespace SquidDraftLeague.MySQL
{
    public static class MySqlClient
    {
        ...
        static MySqlClient()
        {
            string connectionString = $"server={Globals.BotSettings.MySqlIp}";
        }
        ...
        public static async Task<DateTime> GetDateOfLastSet(SdlPlayer player)
        {
            ...
            return records.Where(x =>
                ((JArray)x.Fields["Alpha Players"]).Any(y => y.Value<string>() == player.AirtableId) ||
                ((JArray)x.Fields["Bravo Players"]).Any(y => y.Value<string>() == player.AirtableId))
                .Select(x => (DateTime)x.Fields["Date"])
        ...
        public static async Task RegisterTeam()
        {
            // Snowflake generator.
        ...
        public static async Task ReportScores(Set set, double gain, double loss)
        {
            using (AirtableBase airtableBase = new AirtableBase(Globals.BotSettings.AppKey, Globals.BotSettings.BaseId))
            {
                ...
                fields.AddField("Date", DateTime.UtcNow);
        ...
        public static async Task PenalizePlayer(ulong discordId, double points, string notes)
        {
